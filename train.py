...
group.add_argument('--throughput', default=False, action='store_true',
                    help='Enable calculate throughput')
group.add_argument('--latency', default=False, action='store_true',
                    help='Enable calculate latency')
group.add_argument('--compute-flops', default=False, action='store_true',
                    help='Enable calculate FLOPs')
group.add_argument('--validate-only', action='store_true', default=False,
...
def main():
    ...
    if args.compute_flops:
        ...
        print('param_count:', round(param_count / 1e6, 2))
        # compute flops using FVCore
        input_tensor = torch.randn((1,) + tuple(data_config['input_size'])).to(args.device)
        ...
        try:
            ...
            '''
            print('flops_fvcore: ', round(flops_fvcore / 1e6, 4))
            
        except Exception:
            pass
    
    if args.latency or args.throughput:
        # layency & throughput
        #input_tensor_t = torch.randn(args.batch_size,3,256,256, dtype=torch.float).to(device)
        input_tensor_t = torch.randn((args.batch_size,) + tuple(data_config['input_size'])).to(args.device)
        ...
        if args.latency:
            ...
            print('OK')
            print('mean_latency: ', round(mean_syn, 2))
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                with open(path, 'a') as f:
                    print('{0}: [{1:>4d}/{2}]  '
                        ...
                        'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
                            log_name, batch_idx, last_idx, batch_time=batch_time_m,
                            loss=losses_m, top1=top1_m, top5=top5_m),file=f)
