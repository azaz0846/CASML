...
class poolAttention(nn.Module):
    ...
    def __init__(self, dim, head_dim=32, num_heads=None, qkv_bias=False,
        ...
        self.projq = nn.Linear(dim, self.attention_dim, bias=qkv_bias)
        self.projkv = nn.Linear(dim, self.attention_dim * 2, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(self.attention_dim, dim, bias=proj_bias)
        self.proj_drop = nn.Dropout(proj_drop)
        self.poolkv = Pooling()
    ...
    def forward(self, x):
        ...
        kv = self.poolkv(x)
        N = H * W
        q = self.projq(x).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        kv = self.projkv(kv).reshape(B, -1, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
...
class MetaFormer(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in range(num_stage):
            ...
            if i != 0:
                self.crossTstages.append(CrossTransformer(dim=dims[i],prev_dim=dims[i-1]))
            cur += depths[i]
...
@register_model
def csasml_s18(pretrained=False, **kwargs):
    ...
    model = MetaFormer(
        ...
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['csasml_s18']
    if pretrained:
...
if has_mmseg and has_mmdet:
    ...
    class csasml_s18_feat(MetaFormer):
        """
